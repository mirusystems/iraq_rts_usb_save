...
public class UsbLabel {
    ...
    public UsbLabel(UsbListEntity p) {
        String psCode = String.valueOf(p.getPcId());
        String numCode = String.valueOf(p.getNumPs());
        qr = String.format(Locale.ENGLISH, "%02d-%s : %s", p.getGovId(), psCode.substring(0, 6), numCode);
        title = App.getTitle();
        setPixels();
    }
    ...
    private void setPixels() {
        ...
        mQrHeight = mQrWidth = LabelUtil.calculatePixels(0, dpi); //11 -> 9
